"""
Take the CTD cast data from the BODC archive for the Port Erin time series and
add it to a simple SQLite database. For this database, the easiest way to
interrogate the data will be by time since we know its position.
...
if __name__ == '__main__':
    ...
    con = sqlite3.connect('./ctd-port-erin.db')
    cur = con.cursor()

    base = '/users/modellers/pica/Data/BODC/ctd/'
    metaDataFiles = (os.path.join(base, 'new_port_erin.csv'))
