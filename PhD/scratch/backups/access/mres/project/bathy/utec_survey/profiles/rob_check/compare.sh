...
set outfile=check.ps

#------------------------------------------------------------------------------#

psbasemap $area1 $proj1 -Ba200f50g25:"Distance along line (m)":/a2f1g1WeSn:"Depth (m) CD"::."Hastings Shingle Bank Transect": -Xc -Y15 -P -K >! $outfile

# plot my transect taken from rob's coordinates
awk '{print $1, $4}' from_robs_coords_2.pts | psxy $area1 $proj1 -O -K -H2 -W1/255/0/0 >> $outfile

# plot rob's data (that which he sent)
cat robs_data.csv | tr "," " " | grep -v a | awk '{print ($2+575),($7-0.35)}' | psxy $area1 $proj1 -O -K -W1/0/200/0 >> $outfile
