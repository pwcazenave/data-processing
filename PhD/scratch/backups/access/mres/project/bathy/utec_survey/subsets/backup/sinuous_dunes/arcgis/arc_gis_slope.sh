...
infile=./sinuous_dunes_slope.xyz
outfile=./arcgis_slope.ps

area=-R583000/584000/97200/98000
proj=-Jx0.014
...
plot(){
   ...
   grdimage $area $proj -P \
      ...
      -Cslope.cpt ${infile%.xyz}.grd -Xc -Y16 -K > $outfile
   psscale -D15.3/5/5/0.4 -B5 -Cslope.cpt -O -K >> $outfile
   pstext $proj $area -O -K -N << TEXT >> $outfile
   584075 97800 12 0 0 1 Slope (@+o@+)
TEXT
   grdimage $area $proj -Cutec.cpt -O -K -Y-14c utec.grd \
      -Ba200f100g200:"Eastings":/a200f100g200:"Northings":WeSn \
      -Iutec_grad.grd >> $outfile
   psscale -D15.3/5/5/0.4 -B1 -Cutec.cpt -O -K >> $outfile
   pstext $proj $area -O -N << TEXT >> $outfile
   584075 97800 12 0 0 1 Depth (m)
TEXT
}
