...
mkgrad(){
   grdgradient -Dc -S./grids/slope_S.grd -G./grids/slope_D.grd \
      ../grids/utec_mask.grd
   grdmath ./grida/slope_S.grd ATAN = ./grids/tmp.grd
...
plot(){
   ...
   grdimage $proj $area -P \
      ...
      > $outfile
   gmtset D_FORMAT %g
   psscale -D13/6/6/0.5 -B5 -Cmag.cpt -O -K >> $outfile
...
TEXT
   ...
   grdimage $proj $area \
      -Ba200f100g200:"Eastings":/a200f100g200:"Northings":WeSn \
      -Cdir.cpt ./grids/slope_D.grd -O -K -Y-14 \
      >> $outfile
   psscale -D13/6/6/0.5 -B60 -Cdir.cpt -O -K >> $outfile
   pstext $proj $area -O -N << TEXT >> $outfile
   582540 94850 12 0 0 1 Direction (@+o@+)
TEXT
