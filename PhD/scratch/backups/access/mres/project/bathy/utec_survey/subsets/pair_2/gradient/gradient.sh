...
plot(){
   ...
   grdimage $proj $area -P \
      ...
      -Cmag.cpt ./grids/slope_degrees.grd -K -Xc -Y17 \
      > $outfile
   gmtset D_FORMAT %g
   psscale -D11.1/5/5/0.5 -B5 -Cmag.cpt -O -K >> $outfile
...
TEXT
   ...
   grdimage $proj $area \
      -Ba200f100g200:"Eastings":/a200f100g200:"Northings":WeSn \
      -Cdir.cpt ./grids/slope_D.grd -O -K -Y-13 \
      >> $outfile
   psscale -D11.1/5/5/0.5 -B60 -Cdir.cpt -O -K >> $outfile
