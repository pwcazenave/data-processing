...
infile=./sand_waves_slope.xyz
outfile=./arcgis_slope.ps

area=-R583250/584250/94250/95250
proj=-Jx0.01
...
plot(){
   ...
   grdimage $area $proj -P \
      ...
      -Cslope.cpt ${infile%.xyz}.grd -Xc -Y17c -K > $outfile
   psscale -D11/4.5/5/0.4 -B5 -Cslope.cpt -O -K >> $outfile
   pstext $proj $area -O -K -N << TEXT >> $outfile
   584340 95000 12 0 0 1 Slope (@+o@+)
TEXT
   grdimage $area $proj -Cutec.cpt -O -K -Y-14c utec_mask.grd \
      -Ba200f100g200:"Eastings":/a200f100g200:"Northings":WeSn \
      -Iutec_grad.grd >> $outfile
   psscale -D11/4.5/5/0.4 -B5 -Cutec.cpt -O -K >> $outfile
   pstext $proj $area -O -N << TEXT >> $outfile
   584340 95000 12 0 0 1 Depth (m)
TEXT
}
