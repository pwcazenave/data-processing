...
infile=./raw_data/doris_1000m_subset_results_errors_asymm.csv
bathyin=./grids/DORISall2m_interp_2m_subset.grd
maskin=./grids/DORISall2m_interp_2m_mask.grd
...
# add in the vectors
awk -F, '{if ($3>2) print $1,$2,$4+90,0.45*(log($3)/log(10))}' $infile | \
...
#0.9 32 12 0 0 1 II
#LABEL
awk -F, '{if ($3>2 && $4<90 && $1>='$west' && $1<='$east' && $2>='$south' && $2<='$north') print $4+90,$4+90+$8,$4+90-$9; else print $4-90,$4-90+$8,$4-90-$9}' ${infile} | tr " " "\n" | \
	pshistogram $darea $hproj -W5 -Ggray -L1 -O -K -T0 -Z1 \
	-Ba90f10:,"-@+o@+":/a15f5:,"-%":WesN -X4.7 >> $outfile
