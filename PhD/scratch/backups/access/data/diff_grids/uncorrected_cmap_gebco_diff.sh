...
cmap_in=../cmap/raw_data/uncorrected_CMAP_bathy.xyz
gebco_in=../gebco/plot/raw_data/gebco_channel.xyz
cmap_grid=./grids/u_cmap_bathy_"$grid_size"m.grd
gebco_grid=./grids/gebco_bathy_"$grid_size"m.grd
outfile=./images/u_cmap_gebco_diff.ps
hist=./images/u_cmap_gebco_histogram.ps
hist_cmap=./images/u_cmap_percentage_diff_histogram.ps
hist_gebco=./images/u_cmap_gebco_percentage_diff_histogram.ps
pc_cmap=./images/u_cmap_percentage_diff_plot.ps
pc_gebco=./images/u_cmap_gebco_percentage_diff_plot.ps
...
grd2xyz -S ./grids/u_cmap_gebco_"$grid_size"m_diff.grd \
   > ./grids/u_cmap_gebco_"$grid_size"m_diff.txt

pshistogram ./grids/u_cmap_gebco_"$grid_size"m_diff.txt \
...
mean=$(
   printf %.2f \
   $(cut -f3 ./grids/u_cmap_gebco_"$grid_size"m_diff.txt | mean.awk)
   )
stddev=$(
   printf %.2f \
   $(cut -f3 ./grids/u_cmap_gebco_"$grid_size"m_diff.txt | std_dev.awk )
   )
range=$(
   printf %.2f \
   $(cut -f3 ./grids/u_cmap_gebco_"$grid_size"m_diff.txt | minmax -C | \
   awk '{print sqrt($1**2)+sqrt($2**2)}')
   )
kurtosis=$(
   printf %.2f \
   $(cut -f3 ./grids/u_cmap_gebco_"$grid_size"m_diff.txt | \
   stat_moments.awk | \
...
awk '{print $1,$2,sqrt(($3-('$mean'))**2)}' \
   ./grids/u_cmap_gebco_"$grid_size"m_diff.txt \
   > ./grids/u_cmap_gebco_"$grid_size"m_diff_abs.txt

# now do the cumulative histogram,
pshistogram ./grids/u_cmap_gebco_"$grid_size"m_diff_abs.txt \
...
grdmath \
   ./grids/u_cmap_gebco_"$grid_size"m_diff.grd \
   ./grids/u_cmap_bathy_"$grid_size"m.grd \
   DIV \
   100 MUL \
   = ./grids/u_cmap_diff_${grid_size}m_percent.grd
# relative to gebco
grdmath \
   ./grids/u_cmap_gebco_"$grid_size"m_diff.grd \
...
grd2xyz -S ./grids/u_cmap_diff_${grid_size}m_percent.grd \
   > ./grids/u_cmap_diff_"$grid_size"m_percent.txt
...
pshistogram ./grids/u_cmap_diff_"$grid_size"m_percent.txt \
...
cmean=$(
   printf %.2f \
   $(cut -f3 ./grids/u_cmap_diff_"$grid_size"m_percent.txt | mean.awk)
   )
cstddev=$(
   printf %.2f \
   $(cut -f3 ./grids/u_cmap_diff_"$grid_size"m_percent.txt | std_dev.awk )
   )
...
awk '{print $1,$2,sqrt(($3-('$cmean'))**2)}' \
   ./grids/u_cmap_diff_"$grid_size"m_percent.txt \
   > ./grids/u_cmap_diff_"$grid_size"m_percent_abs.txt
pshistogram ./grids/u_cmap_diff_"$grid_size"m_percent_abs.txt \
...
grdimage $area $proj ./grids/u_cmap_diff_"$grid_size"m_percent.grd \
   -Ba1f0.5g1WeSn \
   -C./pc_diff.cpt \
   -K -Xc -Yc > $pc_cmap
