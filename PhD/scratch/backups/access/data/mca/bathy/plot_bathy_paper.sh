...
#   makecpt $(grdinfo -T1 ./grids/$(basename ${ingrd%.*}.grd)) -Cgray > ./cpts/$(basename ${ingrd%.*}.cpt)
   makecpt -T0/60/0.1 -I -Cgray > ./cpts/$(basename ${ingrd%.*}.cpt)

   gmtset D_FORMAT=%.0f
   psbasemap $area $proj -Xc -Yc -K -B0 > $outfile
   grdimage $area $proj -Xc -Yc -K ./grids/$(basename ${ingrd%.*}.grd) \
      -C./cpts/$(basename ${ingrd%.*}.cpt) -I./grids/$(basename ${ingrd%.*}_grad.grd) \
...
TEXT
   ...
   psscale -D22/7/-7/0.5 -B10:"Depth (m)": -C./cpts/$(basename ${ingrd%.*}.cpt) -I -O -K >> $outfile
