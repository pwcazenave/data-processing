...
# script to make a 200m grid of the seazone data
...
proj=-JM15
gres=200e # match the cd2msl grid resolution

#infile=./subset_larger.xyz
infile=./bathy.xyz
cpt=./seazone.cpt
outfile=./images/seazone_subset_larger.ps

mkregular(){
   echo -n "blockmean and xyz2grd... "
   gmtset D_FORMAT=%.6f
...
mkmask(){
   echo -n "make a mask... "
   grdmask $area $infile -I$gres -NNaN/1/1 -S$gres \
