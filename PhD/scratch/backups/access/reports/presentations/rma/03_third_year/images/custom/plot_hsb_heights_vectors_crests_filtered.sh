...
outfile=./images/$(basename ${infile%.*}_heights_vectors_crests_filtered.ps)
...
# add in the vectors
awk -F, '{if ($3>2 && $4>45 && $4<70) print $1,$2,$4+90,0.5*(log($3)/log(10))}' $infile | grep -v NaN | \
   psxy $parea $proj -O -K -SVb0/0/0 -W5/255/255/255 -Gwhite >> $outfile
#awk -F, '{if ($3>8 && $3<30 && $4>120 && $4<160) print $1,$2,$4,0.3*(log($3)/log(10))}' $infile | \
#   psxy $parea $proj -O -K -SVb0/0/0 -W5/255/255/255 -Gwhite >> $outfile
# add in a key
echo "578750 98450 90 20" | awk '{print $1,$2,$3,0.4*(log($4)/log(10))}' | \
...
awk -F, '{if ($3>2 && $4<=90 && $4>45 && $4<70) print $5; else if ($3>2 && $4>90 && $4>55 && $4<75) print $5}' $infile | grep -v NaN | \
...
#0.9 32 12 0 0 1 II
#LABEL
awk -F, '{if ($3>2 && $4<=90 && $4>45 && $4<70) print $4+90,$4+$8+90,$4-$9+90; else if ($3>2 && $4>90 && $4>55 && $4<75) print $4-90,$4+$8-90,$4-$9-90}' $infile | \
	tr " " "\n" | grep -v NaN | \
   pshistogram $darea $hproj -W5 -Ggray -L1 -O -K -T0 -Z1 \
   -Ba180f20:,"-@+o@+":/a20f10:,"-%":wEsN -X4.2 >> $outfile
#pstext $darea $hproj -O -K << LABEL >> $outfile
#155 8 12 0 0 1 III
#LABEL
awk -F, '{if ($3>2 && $4<=90 && $4>45 && $4<70) print $3; else if ($3>2 && $4>90 && $4>55 && $4<75) print $3}' $infile | grep -v NaN | \
