...
#   grdgradient $a481sub1 -Nt0.1 -E15/50 -G${a481sub1%.*}_grad.grd
   ...
   gmtset D_FORMAT=%g
   # add in the vectors
   awk -F, '{if ($3>2 && $14!="NaN" && $15!=0) print $1,$2,$14,2.5}' $a481vectors | \
...
#   makecpt $(grdinfo -T5 $a481sub3) -Crainbow -Z > $a481cpt3
   ...
   gmtset D_FORMAT=%g
   # add in the vectors
#   awk -F, '{if ($3>4 && $14 != "NaN" && $15!=0) print $1,$2,($14-90)+$16,1.85}' $a481vectors3 | \
#      psxy $a481area3 $a481proj3 -O -K -SVb0/0.5/0.15 -W5,gray -Ggray >> $a4813
#   awk -F, '{if ($3>4 && $14 != "NaN" && $15!=0) print $1,$2,($14-90)-$16,1.85}' $a481vectors3 | \
#      psxy $a481area3 $a481proj3 -O -K -SVb0/0.5/0.15 -W5,gray -Ggray >> $a4813
   awk -F, '{if ($3>4 && $14 != "NaN" && $15!=0) print $1,$2,$14-90,1.85}' $a481vectors3 | \
