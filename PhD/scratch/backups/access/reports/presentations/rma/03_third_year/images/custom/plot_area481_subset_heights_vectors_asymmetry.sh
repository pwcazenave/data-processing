...
#   grdgradient $a481sub1 -Nt0.1 -E15/50 -G${a481sub1%.*}_grad.grd
   ...
   gmtset D_FORMAT=%g
   # add in the vectors
   awk -F, '{if ($3>2 && $14!="NaN") print $1,$2,$14,2.5}' $a481vectors | \
      psxy $a481area1 $a481proj1 -O -K -SVb0/0.5/0.15 -W5/255/255/255 -Gwhite >> $a4811
   # add in a key
   psscale -D12.5/10/5/0.5 -Ba5:"Depth (m)": -I -C$a481cpt1 -O -K >> $a4811
   echo "613750 5624580 90 10" | awk '{print $1,$2,$3,0.2*$4}' | \
      psxy $a481area1 $a481proj1 -O -K -SVb0/0.5/0.15 -W5,black -Gblack -N >> $a4811
   echo "613750 5624580 90 10" | awk '{print $1,$2,$3,0.2*$4}' | \
      psxy $a481area1 $a481proj1 -O -K -SVb0/0.5/0.15 -Sc0.1 -W5,black -Gblack -N >> $a4811
...
#   grdgradient $a481sub2 -Nt0.1 -E15/50 -G${a481sub2%.*}_grad.grd
   ...
   gmtset D_FORMAT=%g
   # add in the vectors
   awk -F, '{if ($3>4 && $14!="NaN") print $1,$2,$14,2.5}' $a481vectors | \
      psxy $a481area2 $a481proj2 -O -K -SVb0/0.5/0.15 -W5/255/255/255 -Gwhite >> $a4812
   # add in a key
   psscale -D14.5/10/5/0.5 -Ba5:"Depth (m)": -I -C$a481cpt2 -O -K >> $a4812
   echo "613750 5624580 90 10" | awk '{print $1,$2,$3,0.2*$4}' | \
      psxy $a481area2 $a481proj2 -O -K -SVb0/0.5/0.15 -W5,black -Gblack -N >> $a4812
   echo "613750 5624580 90 10" | awk '{print $1,$2,$3,0.2*$4}' | \
      psxy $a481area2 $a481proj2 -O -K -SVb0/0.5/0.15 -Sc0.1 -W5,black -Gblack -N >> $a4812
...
#   makecpt $(grdinfo -T5 $a481sub3) -Crainbow -Z > $a481cpt3
   grdgradient $a481sub3 -Nt0.1 -E15/50 -G${a481sub3%.*}_grad.grd
   gmtset D_FORMAT=%.0f
   ...
   gmtset D_FORMAT=%g
   # add in the vectors
   awk -F, '{if ($3>4 && $14 != "NaN") print $1,$2,($14-90)+$16,1.85}' $a481vectors3 | \
      psxy $a481area3 $a481proj3 -O -K -SVb0/0.5/0.15 -W5,gray -Ggray >> $a4813
   awk -F, '{if ($3>4 && $14 != "NaN") print $1,$2,($14-90)-$16,1.85}' $a481vectors3 | \
      psxy $a481area3 $a481proj3 -O -K -SVb0/0.5/0.15 -W5,gray -Ggray >> $a4813
   awk -F, '{if ($3>4 && $14 != "NaN") print $1,$2,$14-90,1.85}' $a481vectors3 | \
      psxy $a481area3 $a481proj3 -O -K -SVb0/0.5/0.15 -W5/255/255/255 -Gwhite >> $a4813
   # add in a key
   psscale -D14.5/10/5/0.5 -Ba5:"Depth (m)": -I -C$a481cpt3 -O -K >> $a4813
   echo "613750 5624580 90 10" | awk '{print $1,$2,$3,0.2*$4}' | \
      psxy $a481area3 $a481proj3 -O -K -SVb0/0.5/0.15 -W5,black -Gblack -N >> $a4813
   echo "613750 5624580 90 10" | awk '{print $1,$2,$3,0.2*$4}' | \
      psxy $a481area3 $a481proj3 -O -K -SVb0/0.5/0.15 -Sc0.1 -W5,black -Gblack -N >> $a4813
