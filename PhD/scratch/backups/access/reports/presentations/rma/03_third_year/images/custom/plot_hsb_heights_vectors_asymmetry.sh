...
outfile=./images/$(basename ${infile%.*}_heights_vectors_asymmetry.ps)
...
# add in the vectors
awk -F, '{if ($3>8) print $1,$2,$14,0.5*(log($3)/log(10))}' $infile | grep -v NaN | \
   psxy $parea $proj -O -K -SVb0/0.3/0.1 -W5/255/255/255 -Gwhite >> $outfile
#awk -F, '{if ($3>8 && $3<30 && $4>120 && $4<160) print $1,$2,$14,0.5*(log($3)/log(10))}' $infile | grep -v NaN | \
...
#0.9 32 12 0 0 1 II
#LABEL
awk -F, '{if ($3>8) print $14,$14+$8,$14-$9}' ${infile} | tr " " "\n" | grep -v NaN | \
