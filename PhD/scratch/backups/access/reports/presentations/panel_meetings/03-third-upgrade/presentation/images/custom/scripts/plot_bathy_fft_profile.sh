...
hsbasymm=../hsb_asymmetry.csv
invasymm=../invincible_asymmetry.csv
dutchasymm=../dutch_asymmetry.csv
...
# profiles
hsbtransarea=$(cut -f3,4 -d, $hsbtrans | minmax -I5/0.15)
invtransarea=$(cut -f3,4 -d, $invtrans | minmax -I5/0.2)
dutchtransarea=$(cut -f3,4 -d, $dutchtrans | minmax -I5/0.2)
...
#makecpt -T$(cut -f3 -d, $hsbbathy | minmax -C | awk '{print $1"/"$2}')/0.01 -Crainbow -Z > $hsbcpt
makecpt $(cut -f3 -d, $hsbbathy | minmax -T0.1) -Crainbow -Z > $hsbcpt
xyz2grd $invbarea -I$invgres $invbathy -G$invgrd
grdgradient -Nt0.7 -A315 $invgrd -G$invgrad
makecpt $(cut -f3 -d, $invbathy | minmax -T0.1) -Crainbow -Z -I > $invcpt
xyz2grd $dutchbarea -I$dutchgres $dutchbathy -G$dutchgrd
grdgradient -Nt0.9 -A270 $dutchgrd -G$dutchgrad
makecpt $(cut -f3 -d, $dutchbathy | minmax -T0.1) -Crainbow -Z > $dutchcpt
# fft
gmtset D_FORMAT=%g
...
psscale -D5.25/2.25/3/0.25 -B$dutchfftint:"Power (m)": -C$dutchfftcpt -O -K >> $outfile
...
psscale -D5.25/2.25/3/0.25 -B$invfftint:"Power (m)": -C$invfftcpt -O -K >> $outfile
...
psscale -D5.25/2.25/3/0.25 -B$hsbfftint:"Power (m)": -C$hsbfftcpt -O -K >> $outfile
...
LABEL
# put asymmetry indicators in
# hard-coded the y values in
awk -F, '{if ($3==1) print $1,0.32,90,0.18}' $dutchasymm | \
   psxy $dutchtransarea $dutchtransproj -SV0.1/0.18/0.1 -W2 -Gblack -O -K >> $outfile
awk -F, '{if ($3==-1) print $1,0.32,270,0.18}' $dutchasymm | \
   psxy $dutchtransarea $dutchtransproj -SV0.1/0.18/0.1 -O -K >> $outfile
awk -F, '{if ($3==0) print $1,0.32,0,0.18}' $dutchasymm | \
   psxy $dutchtransarea $dutchtransproj -SV0.1/0.18/0.1 -W2 -D0/-0.1 -Ggrey -O -K >> $outfile
...
# put asymmetry indicators in
# hard-coded the y values in
awk -F, '{if ($3==1) print $1,0.15,90,0.18}' $invasymm | \
   psxy $invtransarea $invtransproj -SV0.1/0.18/0.1 -W2 -Gblack -O -K >> $outfile
awk -F, '{if ($3==-1) print $1,0.15,270,0.18}' $invasymm | \
   psxy $invtransarea $invtransproj -SV0.1/0.18/0.1 -O -K >> $outfile
awk -F, '{if ($3==0) print $1,0.15,0,0.18}' $invasymm | \
   psxy $invtransarea $invtransproj -SV0.1/0.18/0.1 -W2 -D0/-0.1 -Ggrey -O -K >> $outfile
...
LABEL
pstext $hsbtransarea $hsbtransproj -N -O -K -D-0.7/0.1 << LABEL >> $outfile
$(echo $hsbtransarea | cut -f2 -d'/') $(echo $hsbtransarea | cut -f3 -d'/') 16 0 0 1 M'
LABEL
# put asymmetry indicators in
# hard-coded the y values in
awk -F, '{if ($3==1) print $1,0.3,90,0.18}' $hsbasymm | \
   psxy $hsbtransarea $hsbtransproj -SV0.1/0.18/0.1 -W2 -Gblack -O -K >> $outfile
awk -F, '{if ($3==-1) print $1,0.3,270,0.18}' $hsbasymm | \
   psxy $hsbtransarea $hsbtransproj -SV0.1/0.18/0.1 -O -K >> $outfile
awk -F, '{if ($3==0) print $1,0.3,0,0.18}' $hsbasymm | \
   psxy $hsbtransarea $hsbtransproj -SV0.1/0.18/0.1 -W2 -D0/-0.1 -Ggrey -O >> $outfile
