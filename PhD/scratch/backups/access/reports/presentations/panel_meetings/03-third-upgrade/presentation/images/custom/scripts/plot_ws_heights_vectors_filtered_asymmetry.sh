...
infile=../ws_200m_subset_results_errors_asymm.csv
...
# add in the vectors
awk -F, '{if ($3>8 && $4>55 && $4<75) print $1,$2,$16,0.3*(log($3)/log(10))}' $infile | grep -v NaN | \
   psxy $parea $proj -O -K -SVb0/0.2/0.05 -W5/255/255/255 -Gwhite >> $outfile
#awk -F, '{if ($3>8 && $3<30 && $16>120 && $16<160) print $1,$2,$16,0.3*(log($3)/log(10))}' $infile | \
...
#0.9 32 12 0 0 1 II
#LABEL
awk -F, '{if ($3>8 && $4>55 && $4<75) print $16,$16+$8,$16-$9}' ${infile} | tr " " "\n" | grep -v NaN | \
