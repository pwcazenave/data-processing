...
infile=../hsb_300m_subset_results_errors_asymm.csv
outfile=./images/$(basename ${infile%.*}_heights_vectors_filtered_asymmetry_dir.ps)
...
# add in the vectors
awk -F, '{if ($3>8 && $4>45 && $4<70) print $1,$2,$16,0.5*(log($3)/log(10))}' $infile | grep -v NaN | \
   psxy $parea $proj -O -K -SVb0/0.3/0.1 -W5/255/255/255 -Gwhite >> $outfile
#awk -F, '{if ($3>8 && $3<30 && $4>120 && $4<160) print $1,$2,$16,0.5*(log($3)/log(10))}' $infile | grep -v NaN | \
#   psxy $parea $proj -O -K -SVb0/0/0 -W5/255/255/255 -Gwhite >> $outfile
# add in a key
echo "578750 98450 90 20" | awk '{print $1,$2,$3,0.5*(log($16)/log(10))}' | grep -v NaN | \
psxy $parea $proj -O -K -SVb0/0.3/0.1 -W5 -Gblack >> $outfile
...
awk -F, '{if ($3>8 && $4>45 && $4<70) print $5}' $infile | \
...
#0.9 32 12 0 0 1 II
#LABEL
awk -F, '{if ($3>8 && $4>45 && $4<70) print $16,$16+$8,$16-$9}' ${infile} | tr " " "\n" | grep -v NaN | \
   pshistogram $darea $hproj -W5 -Ggray -L1 -O -K -T0 -Z1 \
   -Ba180f20:,"-@+o@+":/a10f2:,"-%":wEsN -X4.2 >> $outfile
#pstext $darea $hproj -O -K << LABEL >> $outfile
#155 8 12 0 0 1 III
#LABEL
awk -F, '{if ($3>2 && $4>45 && $4<70) print $3}' $infile | grep -v NaN | \
   pshistogram $warea $hproj -W1 -Ggray -L1 -O -Z1 \
   -Ba10f5:,"-m":/a10f2:,"-%":wEsN -Y3.1 >> $outfile
