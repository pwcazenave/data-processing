...
outfile=./images/$(basename ${infile%.*}_heights_vectors_all_asymm.ps)
...
# add in the vectors
awk -F, '{if ($3>8) print $1,$2,$16,0.5*(log($3)/log(10))}' $infile | grep -v NaN | \
