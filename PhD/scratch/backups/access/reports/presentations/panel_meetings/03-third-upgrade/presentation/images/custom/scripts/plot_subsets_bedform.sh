...
#   grdcut $hsbarea1 $hsbbathy -G$hsbsub1
   ...
   # add in the vectors
   awk -F, '{if ($3>2 && $15==1) print $1,$2,$14,0.1*$3}' $hsbvectors | grep -v NaN | \
      psxy $hsbarea1 $hsbproj1 -O -K -SVb0/0.3/0.15 -W5,white -Gwhite >> $hsb1
   # add in a key
   psscale -D23/7.15/5/0.5 -Ba2:"Depth (m)": -I -C$hsbcpt1 -O -K >> $hsb1
   echo "579250 97100 90 10" | awk '{print $1,$2,$3,0.1*$4}' | \
      psxy $hsbarea1 $hsbproj1 -O -K -SVb0/0.3/0.15 -W5,black -Gblack -N >> $hsb1
...
#   grdcut $hsbarea2 $hsbbathy -G$hsbsub2
   ...
   # add in the vectors
   awk -F, '{if ($3>2 && $15==1) print $1,$2,$14,0.15*$3}' $hsbvectors | grep -v NaN | \
      psxy $hsbarea2 $hsbproj2 -O -K -SVb0/0.3/0.15 -W5,white -Gwhite >> $hsb2
   # add in a key
   psscale -D16/7.5/5/0.5 -Ba5:"Depth (m)": -I -C$hsbcpt2 -O -K >> $hsb2
   echo "585200 97100 90 10" | awk '{print $1,$2,$3,0.15*$4}' | \
      psxy $hsbarea2 $hsbproj2 -O -K -SVb0/0.3/0.15 -W5,black -Gblack -N >> $hsb2
...
#   grdcut $mcaarea1 $mcabathy -G$mcasub1
   ...
   gmtset D_FORMAT=%g
   # add in the vectors
   awk -F, '{if ($3>2 && $15==1) print $1,$2,$14,0.2*$3}' $mcavectors | grep -v NaN | \
      psxy $mcaarea1 $mcaproj1 -O -K -SVb0/0.3/0.15 -W5,white -Gwhite >> $mca1
   # add in a key
   psscale -D21.5/6.25/-5/0.5 -Ba5:"Depth (m)": -I -C$mcacpt1 -O -K >> $mca1
   echo "613750 5624580 90 10" | awk '{print $1,$2,$3,0.2*$4}' | \
      psxy $mcaarea1 $mcaproj1 -O -K -SVb0/0.3/0.15 -W5,black -Gblack -N >> $mca1
...
#   grdgradient $mcasub2 -Nt0.1 -E250/50 -G${mcasub2%.*}_grad.grd
   ...
   gmtset D_FORMAT=%g
   # add in the vectors
   awk -F, '{if ($3>2 && $15==1) print $1,$2,$14,0.08*$3}' $mcavectors | grep -v NaN | \
      psxy $mcaarea2 $mcaproj2 -O -K -SVb0/0.3/0.15 -W5,white -Gwhite >> $mca2
   # add in a key
   psscale -D14/8/-5/0.5 -Ba2:"Depth (m)": -I -C$mcacpt2 -O -K >> $mca2
   echo "609150 5619500 90 10" | awk '{print $1,$2,$3,0.2*$4}' | \
      psxy $mcaarea2 $mcaproj2 -O -K -SVb0/0.3/0.15 -W5,white -Gblack >> $mca2
