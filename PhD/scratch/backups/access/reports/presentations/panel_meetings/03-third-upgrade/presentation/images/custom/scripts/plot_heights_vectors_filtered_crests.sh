...
# add in the vectors
awk -F, '{if ($3>8 && $4>45 && $4<70) print $1,$2,$4+90,0.5*(log($3)/log(10))}' $infile | \
   psxy $parea $proj -O -K -SVb0/0/0 -W5/255/255/255 -Gwhite >> $outfile
#awk -F, '{if ($3>8 && $3<30 && $4>120 && $4<160) print $1,$2,$4,0.5*(log($3)/log(10))}' $infile | \
#   psxy $parea $proj -O -K -SVb0/0/0 -W5/255/255/255 -Gwhite >> $outfile
# add in a key
...
#0.9 32 12 0 0 1 II
#LABEL
awk -F, '{if ($3>8 && $4>45 && $4<70) print $4+90,$4+$8+90,$4-$9+90}' ${infile%.csv}_errors.csv | tr " " "\n" | \
   pshistogram $darea $hproj -W5 -Ggray -L1 -O -K -T0 -Z1 \
   -Ba90f10:,"-@+o@+":/a15f5:,"-%":wEsN -X4.2 >> $outfile
#pstext $darea $hproj -O -K << LABEL >> $outfile
#155 8 12 0 0 1 III
#LABEL
awk -F, '{if ($3>2 && $4>45 && $4<70) print $3}' $infile | \
