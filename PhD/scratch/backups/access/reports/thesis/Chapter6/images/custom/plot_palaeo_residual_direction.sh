...
baseDir=./raw_data/
infile=palaeo_residual_direction_0-8k.csv
inmag=palaeo_residual_0-8k.csv
...
base=palaeo_residual_transport_direction_bed_panels
...
mkTri(){
    ...
    awk -F, '{print $1,$2,$'$currCol'}' $rawinmesh | \
...
#    grd2cpt ./grids/$inmesh.grd -L-2000/0 -Crainbow -Z > ./cpts/$inmesh.cpt
    makecpt -T-180/180/1 -Ccyclic -Z > ./cpts/${inmesh}_${currYr}.cpt
...
#    psxy $area $proj -m -A ./raw_data/$inmesh.xy -O -K >> $outfile
    awk -F, '{if ($'$currCol'!=0) print $1,$2,$3}' $baseDir/$inmag | \
        psmask $area $proj -I10m -N -Gwhite -O -K >> $outfile
...
psscale -D-0.5/16.75/4/0.3 -Ba90f30:,-"@+o@+": -O -K -C./cpts/${inmesh}_${currYr}.cpt --D_FORMAT=%lg --LABEL_OFFSET=1.2c >> $outfile
