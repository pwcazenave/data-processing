...
infile=./raw_data/palaeo_stress_points_0-8k.csv
inpc=./raw_data/palaeo_stress_percentages_0-8k.csv
...
plotGraph(){
    timeaxis=0/8000
    yrs=($(seq 0 500 8000))
    garea=-R${timeaxis}/$(awk -F, '{if (NR>1) print $0}' $infile | cut -f2- -d, | tr "," "\n" | minmax -C | awk '{printf "%.1f/%.1f\n", $1, $2}')
    sites=($(awk -F, '{print $3}' $inlocations))

    psbasemap $garea $gproj -Ba2000f500:"Years BP":/a0.1f0.02:"Shear stress (N m@+-2@+)":wEsN -O -K -X10c -Y5.1c >> $outfile
    ...
    200 0.025 16 0 0 1 B
LABEL

    garea=-R${timeaxis}/0/230

    psbasemap $garea $gproj -Ba2000f500:"Years BP":/a50f10:,-%:wESn -O -K -Y-5.1c >> $outfile
    for ((i=0; i<${#sites[@]}; i++)); do
        # Get a line colour
        colour=$(awk '{if (NR=='$(($i+5))') printf "%i/%i/%i\n", ($2+$6)/2,($3+$7)/2,($4+$8)/2}' ./cpts/$base.cpt)
