...
outfile=./images/hsb_group9-10_M32_currents.ps

# Plot extents
area9=-R2005-06-07T00:00:00/2005-06-11T00:00:00/0/2
area9dir=-R2005-06-07T00:00:00/2005-06-11T00:00:00/-180/180
area10=-R2005-06-20T00:00:00/2005-06-24T00:00:00/0/2
...
makePanels(){
    ...
    plotData $area9 $area9dir $xOff $yOff ${group9speed[1]} ${group9dir[1]} calibgroup9_HM32_SM32_d50_var_microns_points.csv ${group9obs[1]}
    # Row 4 (group 9, site 2)
    psbasemap $area9 $proj -Ba2Df6Hg12H/a0.5f0.1:"ms@+-1@+":WeSn -O -K -X-${xOff}c -Y${yOff}c -P >> $outfile
    plotData $area9 $area9dir $xOff $yOff ${group9speed[0]} ${group9dir[0]} calibgroup9_HM32_SM32_d50_var_microns_points.csv ${group9obs[0]}
    # Row 3 (group 10, site 1)
    psbasemap $area10 $proj -Ba2Df6Hg12H/a0.5f0.1:"ms@+-1@+":WeSn -O -K -X-${xOff}c -Y${yOff}c -P >> $outfile
    plotData $area10 $area10dir $xOff $yOff ${group10speed[0]} ${group10dir[0]} calibgroup10_HM32_SM32_d50_var_microns_points.csv ${group10obs[0]}
