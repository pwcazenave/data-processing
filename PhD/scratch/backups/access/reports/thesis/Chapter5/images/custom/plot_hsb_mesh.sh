...
infile=$HOME/modelling/data/bathymetry/meshes/round_11_hsb/local_transport/mesh/combo_v7.mesh
...
plotMap(){
    makecpt -T-60/0/0.5 -Crainbow > ./cpts/${base}.cpt

    # Add the mesh
    awk  '{if (NF==5) print $2,$3,$4}' $infile | \
        pscontour $area $proj -C./cpts/${base}.cpt -I -K -X3c -Y4c > $outfile
    ...
    psscale -C./cpts/$base.cpt -D12.5/-1.2/9/0.5h -Ba10f2:"Depth (m)": -O -K >> $outfile
