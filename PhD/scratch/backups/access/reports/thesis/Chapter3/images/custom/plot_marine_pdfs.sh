...
infileHeight=./raw_data/pdfs/marine_height_pdfs.csv
infileWavelength=./raw_data/pdfs/marine_lambda_pdfs.csv

outfile=./images/marine_bedform_pdfs.ps
...
plotheight(){
	psbasemap $area $proj -X3 -Y18c -K -P \
		-Ba1f0.2:"Dimensionless height":/a0.5f0.1:"Probability density":WeSn > $outfile
	# Observed
...
plotwavelength(){
	psbasemap $area $proj -X8c -O -K \
		-Ba1f0.2:"Dimensionless wavelength":/a0.5f0.1:"Probability density":wESn >> $outfile
	# Wavelength
	# Observed
