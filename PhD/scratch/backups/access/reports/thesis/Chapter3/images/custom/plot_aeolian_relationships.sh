...
outfile=./images/predicted_aeolian_relationship.ps
...
#		psxy $area $proj -Ex -Sh0.25 -O -K -W5,lightred >> $outfile
#	awk -F, '{if ($3>2 && $15==1) print $3,$5,($8+$9)/2}' $jibs2500 | \
#		psxy $area $proj -Ex -Sh0.25 -O -K -W5,lightred >> $outfile
#	awk -F, '{if ($3>2 && $15==1) print $3,$5,($8+$9)/2}' $hsb | \
#		psxy $area $proj -Ex -St0.25 -O -K -W5,darkblue >> $outfile
#	awk -F, '{if ($3>2 && $4>55 && $4<65) print $3,$5,($8+$9)/2,0.06}' $ws | \
#		psxy $area $proj -Exy -Ss0.25 -O -K -W5,darkred >> $outfile
#	awk -F, '{if ($3>4 && $4>0 && $4<30) print $3,$5,($8+$9)/2}' $a4811 | \
#		psxy $area $proj -Ex -Sd0.25 -O -K -W5,darkgreen >> $outfile
...
#		psxy $area $proj -Ex -Sd0.25 -O -K -W5,darkgreen >> $outfile
	awk -F, '{if ($3>180 && $15==1) print $3,$5,($8+$9)/2,5}' $taklamakandesert | \
		psxy $area $proj -Exy -Sh0.2 -O -K -W5,purple >> $outfile
	awk -F, '{if ($3>180 && $15==1) print $3,$5,($8+$9)/2,5}' $srtm30 | \
		psxy $area $proj -Exy -Sd0.2 -O -K -W5,blue -Gblue >> $outfile
#	awk -F, '{if ($3>40 && $15==1) print $3,$5,($8+$9)/2}' $thames | \
#		psxy $area $proj -Ex -S-0.25 -O -K -W5,lightblue >> $outfile
#	awk -F, '{if ($3>40 && $15==1) print $3,$5,($8+$9)/2}' $southernnorthsea | \
#		psxy $area $proj -Ex -S-0.25 -O -K -W5,lightblue >> $outfile
#	awk -F, '{if ($3>4 && $15==1) print $3,$5,($8+$9)/2}' $culver2009 | \
#		psxy $area $proj -Ex -S+0.25 -O -K -W5,darkyellow >> $outfile
#	awk -F, '{if ($3>4 && $15==1) print $3,$5,($8+$9)/2}' $culver2010 | \
#		psxy $area $proj -Ex -S+0.25 -O -K -W5,200/0/100 >> $outfile
	awk -F, '{if ($3>180 && $15==1) print $3,$5,($8+$9)/2,5}' $simpsondesert | \
		psxy $area $proj -Exy -Sd0.2 -O -K -W5,green -Ggreen >> $outfile
	awk -F, '{if ($3>180 && $15==1) print $3,$5,($8+$9)/2,5}' $badainjaran | \
		psxy $area $proj -Exy -Sd0.2 -O -K -W5,red >> $outfile
#	awk -F, '{if ($3>400 && $15==1) print $3,$5,($8+$9)/2,5}' $britned | \
#		psxy $area $proj -Exy -S+0.25 -O -K -W5,100/0/200 >> $outfile
...
250 950
LINE
	psxy $area $tproj -O -K -Sc0.25 -W5,gray << LINE >> $outfile
$xpos2 950
LINE
}
...
##		psxy $dwarea $proj -Ey -Sh0.25 -O -K -W5,lightred >> $outfile
##	awk -F, '{if ($3>2 && $15==1) print sqrt($10^2),$3,($8+$9)/2}' $jibs2500 | \
##		psxy $dwarea $proj -Ey -Sh0.25 -O -K -W5,lightred >> $outfile
##	awk -F, '{if ($3>2 && $15==1) print sqrt($10^2),$3,($8+$9)/2}' $hsb | \
##		psxy $dwarea $proj -Ey -St0.25 -O -K -W5,darkblue >> $outfile
##	awk -F, '{if ($3>2 && $4>55 && $4<65) print sqrt($10^2),$3,($8+$9)/2}' $ws | \
##		psxy $dwarea $proj -Ey -Ss0.25 -O -K -W5,darkred >> $outfile
##	awk -F, '{if ($3>4 && $4>0 && $4<30) print sqrt($10^2),$3,($8+$9)/2}' $a4811 | \
##		psxy $dwarea $proj -Ey -Sd0.25 -O -K -W5,darkgreen >> $outfile
...
##		psxy $dharea $proj -S-0.25 -O -K -W5,lightblue >> $outfile
##	awk -F, '{if ($3>2 && $15==1) print sqrt($10^2),$5}' $jibs350 | \
##		psxy $dharea $proj -Sh0.25 -O -K -W5,lightred >> $outfile
##	awk -F, '{if ($3>2 && $15==1) print sqrt($10^2),$5}' $jibs2500 | \
##		psxy $dharea $proj -Sh0.25 -O -K -W5,lightred >> $outfile
##	awk -F, '{if ($3>2 && $15==1) print sqrt($10^2),$5}' $hsb | \
##		psxy $dharea $proj -St0.25 -O -K -W5,darkblue >> $outfile
##	awk -F, '{if ($3>2 && $4>55 && $4<65) print sqrt($10^2),$5,0.06}' $ws | \
##		psxy $dharea $proj -Ey -Ss0.25 -O -K -W5,darkred >> $outfile
##	awk -F, '{if ($3>4 && $4>0 && $4<30) print sqrt($10^2),$5}' $a4811 | \
##		psxy $dharea $proj -Sd0.25 -O -K -W5,darkgreen >> $outfile
...
#		psxy $dharea $proj -Ey -Sg0.25 -O -K -W5,purple >> $outfile
#	awk -F, '{if ($3>180 && $15==1) print sqrt($10^2),$5,5}' $srtm30 | \
#		psxy $dharea $proj -Ey -S+0.25 -O -K -W5,black >> $outfile
##	awk -F, '{if ($3>40 && $15==1) print sqrt($10^2),$5}' $thames | \
##		psxy $dharea $proj -S-0.25 -O -K -W5,lightblue >> $outfile
##	awk -F, '{if ($3>4 && $15==1) print sqrt($10^2),$5}' $culver2009 | \
##		psxy $dharea $proj -S+0.25 -O -K -W5,darkyellow >> $outfile
##	awk -F, '{if ($3>4 && $15==1) print sqrt($10^2),$5}' $culver2010 | \
##		psxy $dharea $proj -S+0.25 -O -K -W5,200/0/100 >> $outfile
#	awk -F, '{if ($3>180 && $15==1) print sqrt($10^2),$5,5}' $simpsondesert | \
#		psxy $dharea $proj -Ey -Sx0.25 -O -K -W5,green >> $outfile
#	awk -F, '{if ($3>180 && $15==1) print sqrt($10^2),$5,5}' $badainjaran | \
#		psxy $dharea $proj -Ey -S+0.25 -O -K -W5,red >> $outfile
##	awk -F, '{if ($3>400 && $15==1) print sqrt($10^2),$5}' $britned | \
##		psxy $dharea $proj -S+0.25 -O -K -W5,100/0/200 >> $outfile
...
#plotlabel 195 darkblue t "Hastings" $outfile
#plotlabel 180 darkred s "West Solent" $outfile
#plotlabel 165 darkgreen d "Area 481" $outfile
plotlabel 180 blue d "Ar Rub' al Khali" $outfile -Gblue
#plotlabel 150 lightblue - "Thames Estuary" $outfile
#plotlabel 135 lightred h "JIBS" $outfile
#plotlabel 120 darkyellow + "Culver Sands" $outfile
#plotlabel 255 200/0/100 + "Culver Sands 2010" $outfile
plotlabel 165 green d "Simpson Desert" $outfile -Ggreen 
plotlabel 150 purple h "Taklamakan Desert" $outfile
plotlabel 135 red d "Badain Jaran" $outfile
#plotlabel 105 100/0/200 + "BritNed" $outfile
...
##plotlabel 180 darkred s "West Solent" $outfile
##plotlabel 165 darkgreen d "Area 481" $outfile
#plotlabel 255 black + "Ar Rub' al Khali" $outfile
##plotlabel 150 lightblue - "Thames Estuary" $outfile
##plotlabel 135 lightred h "JIBS" $outfile
##plotlabel 120 darkyellow + "Culver Sands" $outfile
##plotlabel 255 200/0/100 + "Culver Sands 2010" $outfile
#plotlabel 240 green x "Simpson Desert" $outfile
#plotlabel 225 purple g "Taklamakan Desert" $outfile
#plotlabel 210 red + "Badain Jaran" $outfile
##plotlabel 105 100/0/200 + "BritNed" $outfile
#
#plotdepthheight
#plotlabel 270 black - "Francken et al. (2004)" $outfile
#plotlabel 255 darkblue t "Hastings" $outfile
##plotlabel 195 darkblue t "Hastings" $outfile
##plotlabel 180 darkred s "West Solent" $outfile
##plotlabel 165 darkgreen d "Area 481" $outfile
#plotlabel 255 black + "Ar Rub' al Khali" $outfile
