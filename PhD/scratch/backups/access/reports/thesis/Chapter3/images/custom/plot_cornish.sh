...
ingrd=./grids/hi1059_b_cut_50m.grd
...
harea=-R0/2/0/10 # height
warea=-R0/3/0/20 # wavelength
darea=-R0/180/0/10 # orientation
adarea=-R0/360/0/10 # asymmetry direction
aarea=-R0/4/0/20 # asymmetry
...
results=./raw_data/hi1059_20000-2500m_subset_results_errors_asymm.csv
...
#	makecpt $(grdinfo -T10 $ingrd) -Z -Crainbow > ./cpts/$(basename ${ingrd%.*}).cpt
	makecpt -T-100/-60/0.5 -Z -Crainbow > ./cpts/$(basename ${ingrd%.*}).cpt
	#grd2cpt $ingrd -Cgray -Z > ./cpts/$(basename ${ingrd%.*}).cpt

	psbasemap $area $proj -X4c -Y12c -K -Ba20000f10000:"Eastings":/a10000f5000:"Northings":WeSn \
		-P > $1
	...
	grdimage $area $proj -C./cpts/$(basename ${ingrd%.*}).cpt -I${ingrd%.*}_grad.grd \
		$ingrd -O -K >> $1
	psscale -D3.25/14.35/-5/0.3h -Ba10f2:"Depth (m)": \
    	--ANNOT_FONT_SIZE=12 \
...
WHITE
    ...
    $midX $startY
BLACK
    pstext $area $proj -D-0.1c/0.4c -O -K -Gwhite << LABEL >> $1
$startX $startY 12 0 0 1 0
$midX $startY 12 0 0 1 5
...
#		psxy $area $proj -O -K -Sc -W4,black >> $1
	...
	awk -F, '{if ($15==1 && $3>24) print $1,$2,$4+90,$3*0.0005}' $results | \
		psxy $area $proj -O -K -SVb0/0/0 -W6,black -Gblack >> $1
    # Asymmetry direction
    awk -F, '{if ($15==1 && $3>24) print $1,$2,$14,$16*0.02}' $results | grep -v NaN | \
...
#		psxy $area $proj -O -K -SVt0/0/0 -W4,black >> $1
	...
	awk -F, '{
                ...
                if ($15==1) {
                    ...
                    print $1,$2,startRot,($5/10),xPos,yPos,-1*xPos,-1*yPos,endRot
                }
...
SYMBOL
        ...
        # Do the plot
        echo $xPos $yPos | \
            psxy $area $proj -O -K -Sksemicircle/${radiusX} -W7,black \
                >> ${outfile%.*}_results.ps
    done < ${results%.*}_symbol.csv

	# Add a scale
    # 1km*0.0005=0.5 (0.0005=scaling factor from above)
	echo "430000 5483800 90 0.5" | \
    	psxy $area $proj -O -K -SVb0/0/0 -W6,black -Gblack >> $1
	echo "430000 5481500 0.25" | \
    	psxy $area $proj -O -K -Sc -W6,black >> $1
	pstext $area $proj -O -K -D0.5/-0.1 -WwhiteO0,white << LABEL >> $1
	430000 5483800 10 0 0 1 1 km wavelength
	430000 5481500 10 0 0 1 0.5 m height
LABEL
}
...
add_scale ${outfile%.*}_results.ps 452000 5481500
