...
ingrd=./grids/srtm_57_04-05_utm_250m.grd
...
area=-R66800/350000/4350000/4600000
proj=-Jx5e-5
...
harea=-R0/100/0/20 # for the results histograms
harea2=-R0/360/0/100 # for the roses
warea=-R0/6/0/15 # wavelength
darea=-R0/180/0/20 # orientation
adarea=-R0/360/0/20 # asymmetry direction
aarea=-R0/4/0/30 # asymmetry
...
results=./raw_data/srtm_45000m_subset_results_errors_asymm.csv
...
#	makecpt $(grdinfo -T10 $ingrd) -Z -Crainbow > ./cpts/$(basename ${ingrd%.*}).cpt
	...
	psbasemap $area $proj -X4c -Y5c -K -Ba100000f25000:"Eastings":/a50000f10000:"Northings":WeSn \
		> $1
	...
	grdimage $area $proj -C./cpts/$(basename ${ingrd%.*}).cpt -I${ingrd%.*}_grad.grd \
		$ingrd -O -K >> $1
	psscale -D4/12/5/0.3h -Ba200f100:"Height (m)": \
    	--ANNOT_FONT_SIZE=12 \
...
#		psxy $area $proj -O -K -Sc -W4,black >> $1
	...
	awk -F, '{if ($15==1 && $3>180) print $1,$2,$4+90,$3*0.0005}' $results | \
		psxy $area $proj -O -K -SVb0/0/0 -W6,black -Gblack >> $1
    # Asymmetry direction
    # Fix for the asymmetry direction here +180
    awk -F, '{if ($15==1 && $3>180) print $1,$2,$14+180,$19*0.2}' $results | grep -v NaN | \
...
#		psxy $area $proj -O -K -SVt0/0/0 -W4,black >> $1
    ...
    # Fix for the asymmetry direction here (flipped $11<$12)
	awk -F, '{
                hyp=0.1
                if ($15==1) {
                    pi=3.141592654
...
#        sleep 1.2
	...
	# Add a scale
    # 2km*0.00015=1 (0.0005=scaling factor from above)
	echo "100000 4520000 90 1" | \
    	psxy $area $proj -O -K -SVb0/0/0 -W6,black -Gblack >> $1
	echo "100000 4535000 0.5" | \
    	psxy $area $proj -O -K -Sc -W6,black >> $1
	pstext $area $proj -O -K -D1/-0.15 -WwhiteO0,white << LABEL >> $1
	100000 4520000 10 0 0 1 2 km wavelength
	100000 4535000 10 0 0 1 50 m height
LABEL
}
...
add_scale ${outfile%.*}_results.ps 280000 4580000
