...
results=./raw_data/jibs_350m_subset_results_errors_asymm.csv
...
#	makecpt $(grdinfo -T10 $ingrd) -Z -Crainbow > ./cpts/$(basename ${ingrd%.*}).cpt
	...
	psbasemap $area $proj -X4c -Y5.5c -K -Ba2000f1000:"Eastings":/a1000f500:"Northings":WeSn \
		> $1
	...
	grdimage $area $proj -C./cpts/$(basename ${ingrd%.*}).cpt -I${ingrd%.*}_grad.grd \
		$ingrd -O -K >> $1
	psscale -D0.5/12/5/0.3 -Ba20f5:"Depth (m)": \
    	--ANNOT_FONT_SIZE=12 \
...
add_scale(){
    ...
    startX=$2
    endX=$(echo "scale=0; $startX+2000" | bc -l)
...
#		psxy $area $proj -O -K -Sc -W4,white >> $1
	...
	awk -F, '{if ($15==1 && $3>4) print $1,$2,$4+90,$3*0.01}' $results | \
		psxy $area $proj -O -K -SVb0/0/0 -W6,white -Gwhite >> $1
    # Asymmetry direction
    awk -F, '{if ($15==1 && $3>4) print $1,$2,$14,$19*0.17}' $results | grep -v NaN | \
...
#		psxy $area $proj -O -K -SVt0/0/0 -W4,white >> $1
	...
	# WARNING: Filtering heights to a maximum of 2m to eliminate
...
SYMBOL
	...
	echo "660500 6127500 90 0.3" | \
    	psxy $area $proj -O -K -SVb0/0/0 -W6,white -Gwhite >> $1
	echo "660500 6127300 0.4" | \
    	psxy $area $proj -O -K -Sc -W6,white >> $1
	pstext $area $proj -O -K -D0.5/-0.15 -WwhiteO0,white << LABEL >> $1
	660500 6127500 10 0 0 1 30 m wavelength
	660500 6127300 10 0 0 1 0.5 m height
LABEL
}
...
add_histograms(){
	...
	awk -F, '{if ($3>2 && $15==1) print $5}' $results | \
		pshistogram $harea $hproj -W0.05 -Ggray -L1,gray -O -K -Z1 \
	   	-Ba0.5f0.1:,"-m":/a10f2:,"-%":WeSn -X0.8c -Y-4.75c >> $1
...
add_scale ${outfile%.*}_results.ps 660200 6128000
