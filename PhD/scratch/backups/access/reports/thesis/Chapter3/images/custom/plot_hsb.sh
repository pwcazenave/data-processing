...
ingrd=./grids/all_lines_blockmedian_5m.grd
...
results=./raw_data/hsb_300m_subset_results_errors_asymm.csv
...
#	makecpt $(grdinfo -T10 $ingrd) -Z -Crainbow > ./cpts/$(basename ${ingrd%.*}).cpt
	makecpt -T-55/-15/0.5 -Z -Crainbow > ./cpts/$(basename ${ingrd%.*}).cpt
	#grd2cpt $ingrd -Cgray -Z > ./cpts/$(basename ${ingrd%.*}).cpt

	psbasemap $area $proj -X3.2c -Y5c -K -Ba2000f1000:"Eastings":/a1000f500:"Northings":WeSn \
		> $1
	...
	grdimage $area $proj -C./cpts/$(basename ${ingrd%.*}).cpt -I${ingrd%.*}_grad.grd \
		$ingrd -O -K >> $1
	psscale -D17/2.6/-5/0.3h -Ba10f2:"Depth (m)": \
    	--ANNOT_FONT_SIZE=12 \
...
#		psxy $area $proj -O -K -Sc -W4,black >> $1
    ...
    # Asymmetry direction
    awk -F, '{if ($15==1 && $3>4) print $1,$2,$14,$19*0.15}' $results | grep -v NaN | \
...
SYMBOL
	...
	echo "586500 93800 90 0.3" | \
    	psxy $area $proj -O -K -SVb0/0/0 -W6,black -Gblack >> $1
	echo "586500 93300 0.25" | \
    	psxy $area $proj -O -K -Sc -W6,black >> $1
	pstext $area $proj -O -K -D0.5/-0.1 -WwhiteO0,white << LABEL >> $1
	586500 93800 10 0 0 1 30 m wavelength
	586500 93300 10 0 0 1 0.25 m height
LABEL
}
