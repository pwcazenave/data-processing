...
harea=-R0/15/0/10 # for the results histograms
harea2=-R0/360/0/100 # for the roses
warea=-R0/10/0/50 # wavelength
darea=-R0/180/0/15 # orientation
adarea=-R0/360/0/10 # asymmetry direction
aarea=-R0/3/0/30 # asymmetry
...
results=./raw_data/britned_50000-20000m_subset_results_errors_asymm.csv

set -eu

prep(){
	grdgradient -Nt1 -A270 $ingrd -G${ingrd%.*}_grad.grd
}
...
#	makecpt $(grdinfo -T10 $ingrd) -Z -Crainbow > ./cpts/$(basename ${ingrd%.*}).cpt
	...
	psbasemap $area $proj -X4c -Y7c -K -Ba40000f10000:"Eastings":/a20000f5000:"Northings":WeSn \
		> $1
	...
	grdimage $area $proj -C./cpts/$(basename ${ingrd%.*}).cpt -I${ingrd%.*}_grad.grd \
		$ingrd -O -K >> $1
	pscoast -R-2/5/50/55 -Jm1 -Df -O -K -W -m | \
    	mapproject -R-2/5/50/55 -Ju31/1 -C -F -m | \
    	psxy $area $proj -O -K -m -W5,128 >> $1
	psscale -D0.5/8/5/0.3 -Ba20f5:"Depth (m)": \
    	--ANNOT_FONT_SIZE=12 \
...
add_scale(){
    ...
    startX=$2
    endX=$(echo "scale=0; $startX+40000" | bc -l)
...
$(($midX+50)) $startY 12 0 0 1 20
$endX $startY 12 0 0 1 40 km
LABEL
}
...
#		psxy $area $proj -O -K -Sc -W4,white >> $1
	...
	awk -F, '{if ($15==1 && $3>4) print $1,$2,$4+90,$3*0.0005}' $results | \
		psxy $area $proj -O -K -SVb0/0/0 -W6,black -Gblack >> $1
    # Asymmetry direction
    awk -F, '{if ($15==1 && $3>4) print $1,$2,$14,$19*0.5}' $results | grep -v NaN | \
...
#		psxy $area $proj -O -K -SVt0/0/0 -W4,white >> $1
	...
	awk -F, '{
                ...
                if ($15==1) {
                    ...
                    print $1,$2,startRot,($5/20),xPos,yPos,-1*xPos,-1*yPos,endRot
                }
...
SYMBOL
	...
	# Add a scale
    # 30m*0.01=0.3 (0.01=scaling factor from above)
	echo "575000 5720000 90 0.3" | \
    	psxy $area $proj -O -K -SVb0/0/0 -W6,black -Gblack >> $1
	echo "575000 5715000 0.25" | \
    	psxy $area $proj -O -K -Sc -W6,black >> $1
	pstext $area $proj -O -K -D0.5/-0.1 -WwhiteO0,white << LABEL >> $1
	575000 5720000 10 0 0 1 30 m wavelength
	575000 5715000 10 0 0 1 0.25 m height
LABEL
}
...
add_scale ${outfile%.*}_results.ps 560500 5700000
