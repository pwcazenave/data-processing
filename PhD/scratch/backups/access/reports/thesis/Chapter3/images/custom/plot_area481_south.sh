...
harea=-R0/5/0/20 # for the results histograms
harea2=-R0/360/0/100 # for the roses
warea=-R50/100/0/40 # wavelength
darea=-R0/180/0/10 # orientation
adarea=-R0/360/0/40 # asymmetry direction
aarea=-R0/15/0/40 # asymmetry
...
outfile=./images/$(basename ${ingrd%.*})_big.ps
results=./raw_data/area481_500-1500m_subset_results_errors_asymm.csv
...
#	makecpt $(grdinfo -T10 $ingrd) -Z -Crainbow > ./cpts/$(basename ${ingrd%.*}).cpt
	makecpt -T-30/-10/0.5 -Z -Crainbow > ./cpts/$(basename ${ingrd%.*}).cpt
	#grd2cpt $ingrd -Cgray -Z > ./cpts/$(basename ${ingrd%.*}).cpt

	psbasemap $area $proj -X3.6c -Y3c -K -Ba500f100:"Eastings":/a500f100:"Northings":WeSn \
		-P > $1
	echo 1 1 1 | xyz2grd $area -I1000 -G./grids/nans.grd
	...
	grdimage $area $proj -C./cpts/$(basename ${ingrd%.*}).cpt -I${ingrd%.*}_grad.grd \
		$ingrd -O -K >> $1
	psscale -D3/2.2/-5/0.3h -Ba5f1:"Depth (m)": \
    	--ANNOT_FONT_SIZE=12 \
...
add_scale(){
    ...
    startX=$2
    endX=$(echo "scale=0; $startX+500" | bc -l)
...
$midX $startY 12 0 0 1 250
$endX $startY 12 0 0 1 500 m
LABEL
}
...
#		psxy $area $proj -O -K -Sc -W4,black >> $1
    ...
    awk -F, '{if ($3>4 && $1>'$west' && $1<'$east' && $2>'$south' && $2<'$north') print $1,$2,$14+90,$3*0.04}' $results | \
		psxy $area $proj -O -K -SVb0/0/0 -W6,white -Gwhite >> $1
    # Asymmetry direction
    awk -F, '{if ($3>4 && $1>'$west' && $1<'$east' && $2>'$south' && $2<'$north') print $1,$2,$14,$19*0.1}' $results | grep -v NaN | \
...
#		psxy $area $proj -O -K -SVt0/0/0 -W4,black >> $1
	...
	awk -F, '{
                ...
                if ($1>'$west' && $1<'$east' && $2>'$south' && $2<'$north') {
                    ...
                    print $1,$2,startRot,($5/5),xPos,yPos,-1*xPos,-1*yPos,endRot
                }
...
SYMBOL
	...
	# Add a scale
    # 20m*0.04=0.8 (0.08=scaling factor from above)
	echo "346230 5901575 90 0.8" | \
    	psxy $area $proj -N -O -K -SVb0/0/0 -W6,white -Gwhite >> $1
	echo "346230 5901700 1" | \
    	psxy $area $proj -N -O -K -Sc -W6,white >> $1
	pstext $area $proj -N -O -K -D0.75/-0.1 -WwhiteO0,white << LABEL >> $1
	346230 5901575 10 0 0 1 20 m wavelength
	346230 5901700 10 0 0 1 1 m height
LABEL
}
...
add_scale ${outfile%.*}_results.ps 346100 5901350
