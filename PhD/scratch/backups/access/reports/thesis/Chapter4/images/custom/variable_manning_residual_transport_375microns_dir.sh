...
infile=./raw_data/calibgroup101_HMvar_SM32_d50_375microns_transport_area_residual_bed.csv
...
plotMap(){
    ...
    # symbol.
#    awk -F"," '{if ($4!=0) print $1,$2,$4,$3,(log($4)/log(10))*-0.01}' $infile | \
#        psxy $area $proj -SVb0.005c/0.025c/0.01cn0.01 -Gblack -O -K -C./cpts/${base}_mag.cpt >> $outfile

    # Mask out the triangulated areas
    awk -F"," '{if ($4!=0) print $1,$2}' $infile | \
...
DOMAIN
    ...
    psscale -D9.3/23.4/6/0.4h -Ba${scaleInt}f${scaleIntSub}:"${varLabel}": -O -K -C./cpts/${base}_dir.cpt >> $outfile
