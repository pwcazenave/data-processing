...
inbathy=../ws_mask_bathy_50m.xyz
outfile=./images/$(basename ${infile%.*}_heights_vectors_filtered.ps)
...
# add in the vectors
awk -F, '{if ($3>2 && $4>55 && $4<75) print $1,$2,$4,0.4*(log($3)/log(10))}' $infile | \
   psxy $parea $proj -O -K -SVb0/0/0 -W5/255/255/255 -Gwhite >> $outfile
#awk -F, '{if ($3>8 && $3<30 && $4>120 && $4<160) print $1,$2,$4,0.4*(log($3)/log(10))}' $infile | \
#   psxy $parea $proj -O -K -SVb0/0/0 -W5/255/255/255 -Gwhite >> $outfile
# add in a key
...
awk -F, '{if ($3>2 && $4>55 && $4<75) print $5}' $infile | \
   pshistogram $harea $hproj -W0.05 -Ggray -L1 -O -K -Z1 \
   -Ba0.5f0.1:,"-m":/a5f1:,"-%":WesN -X16 -Y0.35 >> $outfile
#pstext $harea $hproj -O -K << LABEL >> $outfile
#0.9 32 12 0 0 1 II
#LABEL
awk -F, '{if ($3>2 && $4>55 && $4<75) print $4,$4+$8,$4-$9}' $infile | tr " " "\n" | \
   pshistogram $darea $hproj -W5 -Ggray -L1 -O -K -T0 -Z1 \
   -Ba90f10:,"-@+o@+":/a10f2:,"-%":wEsN -X4.2 >> $outfile
#pstext $darea $hproj -O -K << LABEL >> $outfile
#155 8 12 0 0 1 III
#LABEL
awk -F, '{if ($3>2 && $4>55 && $4<75) print $3}' $infile | \
