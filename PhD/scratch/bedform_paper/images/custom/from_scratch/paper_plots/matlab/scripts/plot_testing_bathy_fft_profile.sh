...
tarea=$(cut -f3,4 -d, $btrans | minmax -I5/0.1)
tproj=-JX13.5/6

# bathy
plot_bathy(){
   gmtset D_FORMAT=%.0f
   psbasemap -P $barea $bproj -K -X4.4 -Y21 \
...
plot_fft(){
   ...
   psscale -D3/-2.3/4/0.25h -C$fcpt -Ba2000f500:."Power": -O -K >> $outfile
}
...
plot_profile(){
   gmtset D_FORMAT=%g
   psbasemap $tarea $tproj -X-7.5 -Y-10 -O -K \
   ...
   pstext $tarea $tproj -N -O -K -D0.1/0.1 << LABEL >> $outfile
0 $(echo $tarea | cut -f3 -d'/') 16 0 0 1 L
LABEL
   pstext $tarea $tproj -N -O -K -D-0.6/0.1 << LABEL >> $outfile
$(echo $tarea | cut -f2 -d'/') $(echo $tarea | cut -f3 -d'/') 16 0 0 1 L'
LABEL
}
