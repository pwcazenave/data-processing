...
gmtset LABEL_FONT_SIZE=20 ANNOT_FONT_SIZE=20
gmtset D_FORMAT=%g
...
#   makecpt $(grdinfo -T1 ./grids/$(basename ${fin%.*}.grd)) -I \
   ...
   makecpt -T1/50000/100 -I \
      -Crainbow > ./cpts/$(basename ${fin%.*}.cpt)
   ...
   echo "0 0 $fphi $(echo "scale=2; $flambda*100" | bc -l)" | psxy $farea $fproj -SV0/0/0 -O -K -W5 -Gblack >> $outfile
   echo "0 0" | psxy $farea $fproj -Sc$(echo "scale=2; 30*$flambda" | bc -l) -O -K -W8 >> $outfile
   psscale -D3/-2.3/4/0.25h -C$fcpt -Ba50000f10000:"Power": -O -K >> $outfile
