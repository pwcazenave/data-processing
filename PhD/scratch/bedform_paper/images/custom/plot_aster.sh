...
prep(){
	grdgradient -Nt2.5 -A160 $ingrd -G${ingrd%.*}_grad.grd
}
...
plot_topo(){
	...
	psscale -D7.7/2.2/3/0.3h -Ba400f100:"Height (m)": \
...
#    snorth=2227000
    ...
    # Overlay box
    psxy $area $proj -O -K -W7,black,- -L -X-11.15 -Y-0.25 << BOX >> $1
    $swest $ssouth
    $swest $snorth
    $seast $snorth
    $seast $ssouth
...
TEXT
    ...
    # Add a scale
    psxy $sarea $sproj -O -K -W10,white << WHITE >> $1
    $(($swest+5000)) $(($ssouth+7000))
    $(($swest+5000+50000)) $(($ssouth+7000))
WHITE
    psxy $sarea $sproj -O -K -W10,black << BLACK >> $1
    $(($swest+5000)) $(($ssouth+7000))
    $(($swest+5000+25000)) $(($ssouth+7000))
BLACK
    pstext $sarea $sproj -D-0.1c/0.15c -O -K -Gwhite << LABEL >> $1
    $(($swest+5000)) $(($ssouth+7000)) 8 0 0 1 0
    $(($swest+5000+25000)) $(($ssouth+7000)) 8 0 0 1 25
    $(($swest+5000+50000)) $(($ssouth+7000)) 8 0 0 1 50 km
LABEL
...
#        -I -O -K --ANNOT_FONT_SIZE=10 --LABEL_FONT_SIZE=10 --ANNOT_OFFSET=0.05c --LABEL_OFFSET=0.05c >> $1
    psscale -D2.9/1.3/2.5/0.25 -Ba25f5:,-m: -C./cpts/$(basename ${ingrd%.*})_subset.cpt \
        -I -O -K --ANNOT_FONT_SIZE=8 --LABEL_FONT_SIZE=8 --ANNOT_OFFSET=0.05c --LABEL_OFFSET=0.05c >> $1
}

add_location(){
	gmtset ANNOT_FONT_SIZE=8 LABEL_FONT_SIZE=8 BASEMAP_TYPE=plain ANNOT_OFFSET=0.05c PLOT_DEGREE_FORMAT=+F
	psbasemap $overallarea $overallproj -Ba10WesN -O -K -X11.15 -Y0.25 >> $1
...
34 14 6 80 0 1 Africa
50 33 6 0 0 1 Eurasia
40 26 6 0 0 1 Arabian
41 23.5 6 0 0 1 Peninsula
56 13 6 0 0 1 Arabian Sea
LABELS
...
plot_wind_locations(){
    ...
    for ((i=0; i<${#windNames[@]}; i++)); do
        ...
        ${windStationsX[i]} ${windStationsY[i]} 10 0 0 MC ${windNames[i]}
LABEL
    done
}
