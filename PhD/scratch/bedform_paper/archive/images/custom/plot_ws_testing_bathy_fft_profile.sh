...
btrans=./raw_data/mca_profile_ifft.csv
bheight=./raw_data/mca_heights.csv
basymm=./raw_data/mca_asymmetry.csv
...
fin=./raw_data/mca_fft.csv
fgrd=./grids/mca_fft.grd
fcpt=./cpts/mca_fft.cpt
fgres=0.0030/0.00239377618192704
...
plot_bathy(){
   gmtset D_FORMAT=%.0f
   psbasemap -P $barea $bproj -K -X4.4 -Y13 \
...
plot_fft(){
   gmtset D_FORMAT=%g
   xyz2grd $farea -I$fgres $fin -G$fgrd
   ...
   psscale -D3/-2.3/4/0.25h -C$fcpt -Ba3000f500:"Power (m)": -O -K >> $outfile
}
